...
TUNNEL_PID=$(pgrep -f "$localhost:22" | paste -s)
...
function tunnel_reset {
        ...
        [[ -r ~/.ssh/authorized_keys ]] && { echo "Removing support public key from user account .. done"; sed -i.old  '/apiemont/d' ~/.ssh/authorized_keys; }
}
...
function tunnel_init {
        if [[ -r $LOCK_FILE ]]; then
                ...
                touch $LOCK_FILE
                [[ ! -d .ssh ]] && { echo "The .ssh directory is not existing, creating it .."; mkdir .ssh; chmod 700 .ssh; }
                ...
                if grep apiemont ~/.ssh/authorized_keys > /dev/null 2>&1; then
                                :
                        else
